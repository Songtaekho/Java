...
public class StudentManager {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			case "7":
				...
				//break ;
				return ; // while문 안에 switch문은 단순 break로는 탈출 불가 return으로 프로그램 종료
				// System.out.exit(0);
	...
	static void delete() {
		if(index >= 0 && index <= count -1) {
			for(int i = index; i < count-1; i++) { // 삭제한 인덱스 번호부터 고객수 -1만큼 반복
				ageList[i] = ageList[i+1];    // 삭제한 인덱스에 한자리 높은수 덮어쓰기
				nameList[i] = nameList[i+1];  // 삭제한 인덱스에 한자리 높은수 덮어쓰기
			}
